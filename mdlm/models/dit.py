...
def _sinusoidal_embedding(values: torch.Tensor,
  ...
  freqs = torch.exp(
    - math.log(max_period)
    * torch.arange(start=0, end=max(half, 1), dtype=torch.float32,
                   device=values.device)
    / max(half, 1))
  args = values[..., None].float() * freqs
  embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1)
  if dim % 2:
    embedding = torch.cat(
      [embedding,
...
class PositionalEncoding3D(nn.Module):
  """
  Sparse 3D sinusoidal positional embeddings for coordinate sequences.
  """
  ...
  def __init__(self, dim: int, max_period: int = 10_000):
    ...
    base_dim = dim // 3
    self.axis_dims = [base_dim, base_dim, dim - 2 * base_dim]

  def forward(self, coords: torch.Tensor) -> torch.Tensor:
    """
    Args:
      coords: Tensor with shape (batch, seq_len, 3) containing xyz coordinates.
    Returns:
      Tensor with shape (batch, seq_len, dim) with sinusoidal embeddings.
    """
    if coords.ndim != 3 or coords.shape[-1] != 3:
      raise ValueError('coords must have shape (batch, seq_len, 3).')

    embeddings = []
    for axis in range(3):
      axis_dim = max(self.axis_dims[axis], 2)
      embeddings.append(_sinusoidal_embedding(
        coords[..., axis], axis_dim, self.max_period))

    return torch.cat(embeddings, dim=-1)[..., :self.dim]
